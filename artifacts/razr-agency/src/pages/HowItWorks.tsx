...
export default function HowItWorks() {
  ...
  return (
    <PageWrapper>
      ...
      <div className="absolute top-1/2 right-0 w-[500px] h-[500px] bg-purple-500/10 rounded-full blur-[140px] pointer-events-none" />
        ...
        <div className="container mx-auto px-4 max-w-4xl text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-black uppercase tracking-tighter leading-[0.9] mb-6">
              ...
              <span className="bg-gradient-to-r from-primary via-purple-400 to-cyan-400 bg-clip-text text-transparent">to live campaigns.</span>
            </h1>
            ...
            <div className="flex flex-wrap justify-center gap-2 mt-10">
              {STEPS.map((s, i) => (
                <div key={i} className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/10 bg-white/[0.03] backdrop-blur text-[10px] font-black uppercase tracking-wider text-white/60">
      ...
      <section ref={containerRef} className="py-16 relative">
        <div className="container mx-auto px-4 max-w-5xl relative">
          {/* Vertical glow line */}
          <div className="absolute left-6 md:left-12 top-0 bottom-0 w-px bg-white/5 rounded-full overflow-hidden">
            <motion.div
              style={{ height: lineHeight }}
              className="absolute top-0 left-0 right-0 bg-gradient-to-b from-primary via-purple-500 to-cyan-400 shadow-[0_0_20px_rgba(0,102,255,0.8)]"
            />
          </div>
      ...
      <section className="py-16 relative">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="relative group">
            <div className="absolute -inset-0.5 bg-gradient-to-r from-primary via-purple-500 to-cyan-500 rounded-3xl blur opacity-40 group-hover:opacity-70 transition-opacity duration-500" />
            <div className="relative rounded-3xl border border-white/10 bg-black/70 backdrop-blur-xl p-10 md:p-14 text-center overflow-hidden">
              <motion.div
                animate={{ x: ["-100%", "200%"] }}
  ...
  return (
    ...
    >
      ...
      <motion.div
        ref={cardRef}
        whileHover={{ y: -4 }}
        className={`relative group rounded-3xl border border-white/10 bg-black/40 backdrop-blur-xl overflow-hidden ${isEven ? "" : "md:ml-auto md:max-w-[92%]"}`}
      >
        ...
        <div className="relative p-6 md:p-10">
          ...
          <p className="text-base md:text-lg text-white/60 leading-relaxed mb-6">{step.desc}</p>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {step.bullets.map((b, idx) => (
              <div
                key={idx}
                className="flex items-center gap-2.5 px-4 py-3 rounded-xl border border-white/5 bg-white/[0.02] text-sm text-white/80"
              >
