...
export default function ProblemSolution() {
  return (
    <section className="relative py-20 md:py-32 overflow-hidden bg-black border-y border-white/5">
      {/* background glows */}
      <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-red-500/[0.06] rounded-full blur-[140px] pointer-events-none" />
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-5 md:gap-8 max-w-6xl mx-auto">
          ...
          <motion.div
            ...
            viewport={{ once: true, margin: "-80px" }}
            transition={{ duration: 0.7 }}
            className="relative p-6 md:p-8 rounded-2xl border border-red-500/20 bg-gradient-to-br from-red-950/30 via-black to-black overflow-hidden"
          >
              ...
              <div className="space-y-5">
                {PAINS.map((p, i) => (
                  <div key={i} className="flex gap-4">
                    <div className="shrink-0 w-6 h-6 rounded-full bg-red-500/15 border border-red-500/40 flex items-center justify-center mt-0.5">
          ...
          <motion.div
            ...
            className="relative p-6 md:p-8 rounded-2xl border border-primary/30 bg-gradient-to-br from-primary/10 via-black to-black overflow-hidden shadow-[0_30px_80px_-20px_rgba(0,102,255,0.3)]"
          >
            <div className="absolute -top-20 -right-20 w-60 h-60 bg-primary/15 rounded-full blur-3xl pointer-events-none" />
                ...
                {GAINS.map((g, i) => (
                  <div key={i} className="flex gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-white/60 leading-relaxed">{g.body}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        >
          ...
          <div className="text-xs uppercase tracking-[0.2em] text-white/40 mt-4">No setup fee • 1-hour activation</div>
        </motion.div>
      </div>
    </section>
  );
