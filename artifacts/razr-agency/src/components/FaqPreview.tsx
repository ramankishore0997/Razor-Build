        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-32">
            ...
            <h2 className="text-4xl md:text-6xl font-black uppercase tracking-tighter leading-[0.95] mb-6">
              Questions, <br />
              <span className="font-light italic text-white/50">answered.</span>
            </h2>
            ...
            <Link
              href="/faq"
              className="inline-flex items-center gap-2 px-5 py-3 rounded-full border border-white/15 bg-white/5 backdrop-blur text-sm font-black uppercase tracking-widest text-white hover:bg-white/10 hover:border-white/30 transition-all group"
            >
              See All FAQ
            ...
            {QS.map((item, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`relative group rounded-2xl border backdrop-blur-xl overflow-hidden transition-all duration-500 ${
                    isOpen
                      ? "border-primary/30 bg-primary/5 shadow-[0_0_30px_rgba(0,102,255,0.15)]"
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-4 min-w-0">
                      <span className={`text-[10px] font-black tracking-widest tabular-nums ${isOpen ? "text-primary" : "text-white/30"}`}>
                        {String(i + 1).padStart(2, "0")}
                      </span>
                      <span className={`text-base md:text-lg font-bold tracking-tight ${isOpen ? "text-white" : "text-white/80"}`}>
                        {item.q}
                      </span>
                    </div>
                    <motion.div
                      ...
                      className={`w-8 h-8 shrink-0 rounded-full border flex items-center justify-center transition-colors ${
                        isOpen ? "bg-primary/20 border-primary/40 text-primary" : "bg-white/5 border-white/10 text-white/60"
                      }`}
                    >
                  ...
                  <AnimatePresence initial={false}>
                    {isOpen && (
                      ...
                      >
                        <div className="px-6 pb-6 pl-[3.75rem] text-white/65 leading-relaxed">
                          {item.a}
                        </div>
                      </motion.div>
                    )}
