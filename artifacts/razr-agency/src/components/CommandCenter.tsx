...
const FEED = [
  ...
  { time: "2s", text: "New conversion: $89.40 ROAS", color: "text-green-400" },
  { time: "8s", text: "Retargeting DPA refresh complete", color: "text-white/70" },
  { time: "14s", text: "Advantage+ exited learning phase", color: "text-yellow-400" },
...
export default function CommandCenter({ className = "" }: { className?: string }) {
  ...
  return (
    <div className={`w-full max-w-3xl rounded-xl border border-white/10 bg-black/50 backdrop-blur-2xl shadow-2xl overflow-hidden ${className}`}>
      {/* Header */}
      ...
      <div className="p-4 sm:p-6 space-y-4 sm:space-y-5">
            ...
            <AreaChart data={data}>
              <defs>
                <linearGradient id="colorSpend" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="hsl(212, 100%, 50%)" stopOpacity={0.35} />
                  <stop offset="95%" stopColor="hsl(212, 100%, 50%)" stopOpacity={0} />
                </linearGradient>
              </defs>
              <Tooltip contentStyle={{ backgroundColor: "#000", border: "1px solid rgba(255,255,255,0.1)", fontSize: 11 }} itemStyle={{ color: "#fff" }} />
              <Area type="monotone" dataKey="spend" stroke="hsl(212, 100%, 50%)" strokeWidth={2} fillOpacity={1} fill="url(#colorSpend)" isAnimationActive={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="rounded-lg bg-white/[0.02] border border-white/5 p-3">
          <div className="flex items-center gap-1.5 mb-2.5">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-white/50">Live Activity</span>
          </div>
          <div className="space-y-1.5 h-16 overflow-hidden relative">
            {FEED.map((f, i) => {
              ...
              return (
                ...
                >
                  <span className="text-white/30 tabular-nums shrink-0 w-8">{f.time}</span>
