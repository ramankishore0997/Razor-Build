...
export default function TrustWall() {
  return (
    <section className="relative w-full overflow-hidden py-14 border-y border-white/5 bg-gradient-to-b from-background via-white/[0.015] to-background">
      ...
      >
        {[...ITEMS, ...ITEMS, ...ITEMS].map((item, i) => {
          ...
          return (
            <div
              key={i}
              className="flex items-center gap-3 px-6 py-3 rounded-full border border-white/10 bg-white/[0.03] backdrop-blur-md shrink-0 hover:border-primary/40 hover:bg-primary/[0.04] transition-colors"
            >
              <Icon className="w-4 h-4 text-primary" />
              <span className="text-sm font-bold uppercase tracking-wider text-white/80">{item.label}</span>
              <span className="w-1.5 h-1.5 rounded-full bg-primary/60 shadow-[0_0_8px_rgba(0,102,255,0.8)]" />
            </div>
          );
        })}
      </motion.div>
